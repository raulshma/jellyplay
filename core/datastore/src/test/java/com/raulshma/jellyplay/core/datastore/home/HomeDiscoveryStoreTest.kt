...
class HomeDiscoveryStoreTest {
    ...
    fun `homeDiscovery flow re-derives on user switch`() = runTest {
        ...
        assertNotNull("user A's slice never reached homeDiscovery", aFlow)

        activate("userB")
        val bFlow = withTimeoutOrNull(10_000) { store.homeDiscovery.first { it.homeMode == HomeMode.VIDEO } }
        assertNotNull("user B's default slice never reached homeDiscovery", bFlow)
    ...
    fun `legacy flat keys are claimed by the first active user`() = runTest {
        ...
        assertEquals("""["legacySeries"]""", raw()[stringPreferencesKey("u_userA::next_up_excluded_series_ids")])
    ...
    fun `no active user reads defaults and writes are skipped`() = runTest {
        ...
        val raw = raw()
        assertTrue(
            "expected no namespaced keys, got: " + raw.asMap().keys.joinToString { it.name },
            raw.asMap().keys.none { it.name.startsWith("u_") },
        )
        assertEquals(HomeMode.VIDEO, slice().homeMode)
    }
