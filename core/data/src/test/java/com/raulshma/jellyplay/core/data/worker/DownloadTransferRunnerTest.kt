 ...
 * writes (status, errorMessage, bytes) + WorkManager [Result] that result,
 ...
 * needed — this is a pure-JVM test like [com.raulshma.jellyplay.core.data.repository.DownloadFailurePolicyTest].
 */
class DownloadTransferRunnerTest {
    ...
    fun `503 on resume zeroes the row bytes and deletes the partial`() = runTest {
        ...
        // was gone, the WorkManager retry would send `Range: bytes=N-`, append a
        // tail to a fresh file, hit the total, and ship a truncated download as
        // COMPLETED. The fix lives in the single-connection applicator, which
        // honours MarkFailed.deletePartial (the transient branch forces it true).
