 ...
 * exercised only through the offline→online transition. Plain JUnit +
 * [MainDispatcherRule] + MockK; the holder runs on its own scope over the
 * test scheduler, mirroring the production viewModelScope hand-off
 * (HomeRefresherTest pattern).
 */
