...
class WyzieSubtitleProviderTest {
    ...
    fun `imdb id preferred over tmdb`() = runBlocking {
        ...
        assertTrue("IMDb id preferred over TMDB", request.path!!.contains("id=tt3659388"))
        assertTrue("TMDB id not sent when IMDb present", !request.path!!.contains("286217"))
        assertTrue("key carried as query param", request.path!!.contains("key=testkey"))
    }
    ...
    fun `tmdb id used when imdb absent`() = runBlocking {
        ...
        val request = server.takeRequest()
        assertTrue("id=286217", request.path!!.contains("id=286217"))
    }
    ...
    fun `non-positive tmdb id is skipped, falls back to imdb`() = runBlocking {
        ...
        val request = server.takeRequest()
        assertTrue("zero tmdb must not be sent", request.path!!.contains("id=tt3659388"))
    }
    ...
    fun `language converted from iso3 to comma-separated iso1`() = runBlocking {
        ...
        val request = server.takeRequest()
        assertTrue("language=en,es", request.path!!.contains("language=en%2Ces"))
    }
