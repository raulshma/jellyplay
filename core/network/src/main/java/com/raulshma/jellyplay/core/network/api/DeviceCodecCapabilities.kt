...
/**
 * Enumerates the device's hardware decoders via
 * [MediaCodecList](`REGULAR_CODECS`) and reports which Jellyfin codec
 * names the platform can decode natively (no software fallback).
 *
 * This is the authoritative source for the hardware-path direct-play
 * profile (ExoPlayer / LibVLC): advertising only codecs the device truly
 * decodes prevents the Jellyfin server from handing back a direct stream
 * that the player then fails to render (e.g. claiming AV1/HEVC 10-bit
 * support on a device whose decoder lacks it). The mime→codec-name map
 * mirrors the Jellyfin naming convention used by the official Android
 * client's `CodecHelpers`.
 *
 ...
 */
@Singleton
open class DeviceCodecCapabilities @Inject constructor() {

    /** Jellyfin video codec names the device decodes in hardware. */
    open val supportedVideoCodecs: Set<String> by lazy { collectCodecs().first }

    /** Jellyfin audio codec names the device decodes in hardware. */
    open val supportedAudioCodecs: Set<String> by lazy { collectCodecs().second }
