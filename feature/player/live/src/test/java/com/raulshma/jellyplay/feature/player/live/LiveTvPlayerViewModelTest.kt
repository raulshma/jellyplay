...
class LiveTvPlayerViewModelTest {
    ...
    fun setUp() {
        ...
        every { fakeEngine.state } returns MutableStateFlow(
            com.raulshma.jellyplay.feature.player.live.engine.LiveEngineState.IDLE
        )
    ...
    fun `resolvePlayback failure surfaces error`() = runTest {
        ...
        kotlinx.coroutines.delay(50)
        assertTrue(vm.state.value.errorMessage?.startsWith("Unable to resolve") == true)
    }
    ...
    fun `empty channel list surfaces error`() = runTest {
        ...
        vm.initialize("ch-0", null, null)
        kotlinx.coroutines.delay(50)
        assertEquals("No channels available", vm.state.value.errorMessage)
