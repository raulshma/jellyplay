 ...
 * the [HttpExecutor] decode/execute bundle, the delete-options query fold,
 * and the two one-line status-only helpers ([deleteRequest], [postEmpty]).
 * Parameterized by `(okHttp, json, serviceName)` — the ONLY textual
 * difference between the two folded copies was the service name inside the
 * user-visible failure strings, so those strings are rebuilt here
 ...
 * need a second parameterization axis for no gain. Both siblings now ride the
 * same [HttpExecutor] chassis — only their shaper lambdas differ.
 *
 * Retry rides the chassis ([HttpExecutor.Options.retryHttpCalls]): every
 * execution this support exposes retries at [HttpExecutor.MAX_RETRIES],
 * replacing the deleted `ResilientRadarrApiClient`/`ResilientSonarrApiClient`
 * DI wrappers.
 */
internal class ArrClientSupport(
    ...
    suspend fun executeRequest(request: Request): Result<String> = http.executeForText(request)
