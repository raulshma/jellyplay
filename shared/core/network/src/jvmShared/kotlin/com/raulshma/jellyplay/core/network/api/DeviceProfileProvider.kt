 ...
 *   [DeviceCodecCapabilities] (a platform codec-enumeration query) rather
...
class DeviceProfileProvider @Inject constructor(
     ...
     * so rebuilding it per `PlaybackInfo` request is pure overhead. Mirrors
     * the memoization of [hardwareProfile].
     */
     ...
     * ExoPlayer handles but that are not reliably surfaced by the platform
     * codec enumeration (notably DTS/MLP/TrueHD, which have no standard
     * mime and are therefore never detected, yet are extremely common in MKV
     * movie rips). Without these, the server transcodes every file carrying
     * such audio. This mirrors the official Jellyfin Android client's
     * `FORCED_AUDIO_CODECS`.
     */
    ...
    private companion object {
         ...
         * recognisable mime (DTS, MLP, TrueHD). Omitting these caused the
         * server to transcode media whose audio the device can in fact decode.
         */
