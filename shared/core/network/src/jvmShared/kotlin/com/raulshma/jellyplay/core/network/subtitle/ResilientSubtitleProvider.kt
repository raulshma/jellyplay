 ...
 *
 * Constructed per provider in the legacy shim's `di/SubtitleProviderModule` —
    ...
    override suspend fun search(
        ...
        .also { outcome ->
            if (outcome.isSuccess) {
                NetworkLog.d(TAG, "${delegate.kind} search ok: ${outcome.getOrThrow().size} result(s)")
            } else {
                val e = outcome.exceptionOrNull()
                NetworkLog.w(TAG, "${delegate.kind} search failed: ${e?.javaClass?.simpleName}: ${e?.message}", e)
            }
        }
