...
/**
 * Surface ratchet for [PlaylistRepository] (the PlaybackRepositorySurfaceTest
 ...
 * coupling its consumers (the music feature's playlist screens, the audio
 * player's playlist picker, the details feature's add-to-playlist targets)
 * pay. This test parses the interface's source, counts its members (fun/val
 * declarations, overloads counted separately — matching the impl's override
 * count), and pins that count so it can only move DOWN.
 *
 * Baseline 8 is the count at the MediaRepository facade split: the eight
 * members moved verbatim from MediaRepositoryImpl to
 * [PlaylistRepositoryImpl] (the six edits' self-invalidation now runs in
 * the extracted impl against the same shared MediaRepositoryInternals
 * detail cluster).
 *
 * Lower [maxInterfaceMembers] when another member retires; never raise it.
 * A genuinely new playlist capability should land as a narrow collaborator
 * over the LibraryApiClient family seam rather than growing this surface.
 */
class PlaylistRepositorySurfaceTest {

    /** The maximum allowed member count of [PlaylistRepository] (see class KDoc). */
    private val maxInterfaceMembers = 8
    ...
    private fun interfaceSource(): File {
        val file = File(
            moduleRoot(),
            "src/commonMain/kotlin/com/raulshma/jellyplay/core/data/repository/PlaylistRepository.kt",
        )
        assertTrue(file.isFile, "PlaylistRepository.kt not found at ${file.path} — the ratchet is vacuous")
        return file
    }
    ...
    /** The comment-stripped body of `interface PlaylistRepository { … }`. */
    private fun interfaceBody(): String {
        val src = interfaceSource().readText(Charsets.UTF_8).stripCommentsAndStrings()
        val header = src.indexOf("interface PlaylistRepository")
        assertTrue(header >= 0, "interface PlaylistRepository declaration not found")
    ...
    fun `interface member count never increases`() {
        ...
        assertTrue(
            ...
            "PlaylistRepository grew to $count members (ratchet baseline $maxInterfaceMembers). " +
                "Retire a member or add a narrow collaborator over the LibraryApiClient family seam " +
                "instead; lower the baseline only when the surface shrinks — never raise it.",
        )
    }
    ...
    fun `interface body parse actually sees members`() {
        ...
        assertTrue(
            countMembers(body) > 0 && body.contains("getPlaylists") && body.contains("movePlaylistItem"),
