...
// (same package). The `@ApplicationContext Context` ctor param was removed —
// it was only ever touched by `android.util.Log`, which now routes through
// the module's Log facade — and the `@Singleton`/`@Inject` annotations were
// stripped: Koin (dataJvmModule) owns construction, and the legacy DataModule
// bridges Hilt injectors (DownloadIntakeImpl) to the single via koin().get().
            ...
            onSuccess = { downloadItem ->
                if (downloadItem.status == com.raulshma.jellyplay.core.model.DownloadStatus.PENDING) {
                    ...
                    try {
                        ...
                        Log.d("DownloadDelegate", "Failed to persist offline images for ${request.mediaItemId}", e)
                    }
                    request.trickplayInfo?.let { info ->
                        try {
                            if (!writer.downloadTrickplayData(request.mediaItemId, info, downloadItem.downloadPath)) {
                                Log.d("DownloadDelegate", "Trickplay sync failed for ${request.mediaItemId}")
                            }
                        } catch (_: Exception) {}
                    }
                    ...
                    if (subtitleStreams.isNotEmpty()) {
                        try {
                            if (!writer.downloadExternalSubtitles(
                                ...
                                Log.d("DownloadDelegate", "Subtitles sync failed for ${request.mediaItemId}")
                            }
                        } catch (_: Exception) {}
                    }
                    try {
                        if (!writer.downloadMediaSegments(request.mediaItemId, downloadItem.downloadPath)) {
                            Log.d("DownloadDelegate", "Segments sync failed for ${request.mediaItemId}")
                        }
                    } catch (_: Exception) {}
                }
