 ...
 * github.com/racra/smooth-corner-rect-android-compose — see AbsoluteSmoothCornerShape.kt.
 */
...
internal class SmoothCorner(
    ...
    private val angleAlpha =
        if (shouldCurveInterpolate)
            degreesToRadians(45.0 * smoothness).toFloat()
        else
            degreesToRadians(45.0 * smoothness * (1 - interpolationMultiplier)).toFloat()

    // Angle which dictates how much of the curve is going to be a slice of a circle
    private val angleBeta =
        if (shouldCurveInterpolate)
            degreesToRadians(90.0 * (1.0 - smoothness)).toFloat()
        else
            degreesToRadians(90.0 * (1 - smoothness * (1 - interpolationMultiplier))).toFloat()

    private val angleTheta = ((degreesToRadians(90.0) - angleBeta) / 2.0).toFloat()
 ...
 */
internal data class ArcSection(
    val radius: Float,
