...
/**
 * Behavior fake for [UserDataMutator], shared by the feature jvmTest suites
 * (details, home). Records every call and mimics the real module's success
 * path — container rewrite (optimistic mode only, like the module), provider
 * session rewrite, series-catalogue drop, provider season rewrite —
 ...
 *
 * Constructed without a [provider] (home's usage) the provider rewrites are
 * no-ops and the observable surface collapses to the played-calls recorder
 * home's former private twin exposed.
 */
class FakeUserDataMutator(
