...
/**
 * JVM actual of the markdown seam (android + desktop): the Material 3
 * `multiplatform-markdown-renderer` wrapper, moved verbatim from commonMain.
 * Covers headings, lists (including GFM task-list checkboxes), fenced code
 * blocks, blockquotes, horizontal rules, tables, inline formatting, and
 * clickable links — links route through the ambient `UriHandler`, opening in
 * the browser.
 *
 * The mikepenz dependencies are jvmShared-scoped (see core/ui build.gradle):
 * their 0.43.0 wasm klibs are Kotlin-2.4-ABI and unreadable by this repo's
 * 2.3.21 loader (spike w-10C class C).
 */
@Composable
