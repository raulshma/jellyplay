...
/**
 * Pins the frozen [LegacyDeferredFetchCoordinator] engine (kept only for the
 * music home — see its class doc): the single-flight/re-arm table (loud
 * failure re-arm, exception-as-failure, cancellation never re-arming, the
 * skip re-arm while a load is in flight, the conservative re-arm when a loud
 * load cancels an in-flight silent regeneration) and the identity decision
 * table. The state-container coordinator's own publish policy lives in
 * [DeferredFetchCoordinatorTest]; this suite must track the legacy engine
 * verbatim until the music home migrates and the class is deleted.
 */
@OptIn(ExperimentalCoroutinesApi::class)
class LegacyDeferredFetchCoordinatorTest {
     ...
     * so [LegacyDeferredFetchCoordinator.load] and silent activations
     * complete synchronously and assertions need no virtual-time plumbing.
     */
    ...
    fun `loud failure re-arms the silent retry on the next activation`() = runTest {
        ...
        val coordinator = LegacyDeferredFetchCoordinator<String>(
    ...
    fun `a silent failure leaves the guard as the last completed fetch set it`() = runTest {
        ...
        val coordinator2 = LegacyDeferredFetchCoordinator<String>(
