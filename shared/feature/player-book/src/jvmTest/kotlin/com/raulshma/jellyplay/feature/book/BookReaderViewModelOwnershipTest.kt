...
/**
 * Ratchet against the reader ViewModel regrowing into a god object (the
 * player-video module's ControllerOwnershipTest pattern): the count of
 * BookReaderViewModel's public + internal members (properties and functions,
 * primary-constructor parameters and private members excluded) must never
 * increase past the post-extraction ceiling. New behaviour belongs in an
 * extracted module built from constructor lambdas — the BookSessionLoader /
 * ReaderProgressReporter / ReaderBookmarkCodec seam shape — with the VM left
 * a thin caller that owns the uiState writes.
 *
 * The extraction baseline: 51 members; the single EPUB event funnel (seven
 * per-event handlers collapsed into one `onEpubEvent`) and the settings
 * sheet's move into the screen's ReaderSheetStack (open/dismiss writers
 * deleted) lowered it to 43. Lower the ceiling when a slice moves out and
...
class BookReaderViewModelOwnershipTest {
    ...
    private fun viewModelSource(): File {
        ...
        return File(dir!!, "src/commonMain/kotlin/com/raulshma/jellyplay/feature/book/BookReaderViewModel.kt")
    }
    ...
    @Test
    fun `bookReaderViewModel public and internal member count never increases`() {
        val file = viewModelSource()
        assertTrue(file.isFile, "BookReaderViewModel.kt not found at ${file.path}")
        ...
        assertTrue(
            ...
            "BookReaderViewModel grew to ${members.size} public+internal members (ceiling " +
                "$maxPublicInternalMembers):\n${members.joinToString("\n") { it.trim() }}\n" +
                "New behaviour belongs in an extracted module (the BookSessionLoader / " +
                "ReaderProgressReporter / ReaderBookmarkCodec seam shape) wired through " +
