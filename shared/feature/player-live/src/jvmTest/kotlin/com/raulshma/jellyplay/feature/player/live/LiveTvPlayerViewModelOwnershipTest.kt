...
/**
 * Ratchet against the ViewModel regrowing into a god object (the library
 * module's LibraryViewModelOwnershipTest pattern): the count of
 * LiveTvPlayerViewModel's public + internal members (properties and
 * functions, primary-constructor parameters and private members excluded)
 * must never increase past the current ceiling. New behaviour belongs in an
 * extracted module built from constructor lambdas — the RecordActions /
...
class LiveTvPlayerViewModelOwnershipTest {
    ...
    private fun viewModelSource(): File {
        ...
        return File(dir!!, "src/commonMain/kotlin/com/raulshma/jellyplay/feature/player/live/LiveTvPlayerViewModel.kt")
    }
    ...
    @Test
    fun `liveTvPlayerViewModel public and internal member count never increases`() {
        val file = viewModelSource()
        assertTrue(file.isFile, "LiveTvPlayerViewModel.kt not found at ${file.path}")
        ...
        assertTrue(
            ...
            "LiveTvPlayerViewModel grew to ${members.size} public+internal members (ceiling " +
                "$maxPublicInternalMembers):\n${members.joinToString("\n") { it.trim() }}\n" +
                "New behaviour belongs in an extracted module (the RecordActions / LiveStreamResolution shape) wired through " +
