...
class HomeViewModelTest {
    ...
    fun markItemPlayed_flipsItemInEverySectionWhereItAppears() = runTest {
        ...
        assertEquals(listOf(Triple<String, Boolean, String?>("cw1", true, null)), userDataMutator.playedCalls)
        val sections = viewModel.uiState.value.sections
     ...
     * the infinite loop. The legacy suite called the VM's public onStop(owner)
     * observer override (app backgrounding); the conveyor move replaced the
     * observer surface with the process-lifecycle seam, so the loop-kill
     * onCleared performs is reproduced here directly: cancelling viewModelScope
     * cancels the refresher's refreshJob child (the loop).
     */
