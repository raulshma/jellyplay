 ...
 * sign-in reset.
 *
 * Harness: MockK + inlined StandardTestDispatcher (Dispatchers.setMain) + runTest.
...
class HomeViewModelTest {
        ...
        runTest {
            try {
                ...
                if (::viewModel.isInitialized) {
                    // The legacy suite called the VM's public onStop(owner)
                    // observer override (app backgrounding); the conveyor move
                    // replaced the observer surface with the process-lifecycle
                    // seam, so the loop-kill onCleared performs is reproduced
                    // here directly: cancelling viewModelScope cancels the
                    // refresher's refreshJob child (the loop).
    ...
    fun markItemPlayed_flipsItemInEverySectionWhereItAppears() = vmTest {
        ...
        assertEquals(listOf(Triple("cw1", true, null as Any?)), userDataMutator.playedCalls)
        val sections = viewModel.uiState.value.sections
    ...
    fun selectSeerrRequestItem_and_clearRequestResult() = vmTest {
        ...
        assertNull(viewModel.uiState.value.seerrRequestState.snapshot.requestResult)
    }
