...
class SubtitleManagerSectionTest {
    ...
    private fun setContent(
        ...
        composeTestRule.setContent {
            MaterialTheme {
                // The hub hoists the tab state with rememberSaveable — without
                // `remember` here every recomposition re-runs mutableIntStateOf(0)
                // and wipes the tab switch, leaving the section stuck on Download.
    ...
    fun subtitleManagerSection_displaysLanguageAndFormat() {
        ...
        // Both fixture rows carry the same provider, so "OpenSubtitles" renders
        // once per row — disambiguate as AVSyncSheetTest does for duplicates.
        composeTestRule.onAllNodesWithText("OpenSubtitles")[0].assertIsDisplayed()
    }
