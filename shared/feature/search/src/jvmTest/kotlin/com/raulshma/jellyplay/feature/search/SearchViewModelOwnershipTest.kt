 ...
 * SearchViewModel's public + internal members (properties and functions,
 ...
 * module built from constructor lambdas — the SeerrRequestStateHolder / DeferredUserDataRefresher / QuickDownloadActions seam shape — with the VM left a thin
 * caller that owns the uiState writes — or, for user intents, in the
 * `SearchUiEvent` fold routed through the single `onEvent` funnel (the
...
class SearchViewModelOwnershipTest {
    ...
    private fun viewModelSource(): File {
        ...
        return File(dir!!, "src/commonMain/kotlin/com/raulshma/jellyplay/feature/search/SearchViewModel.kt")
    }
    ...
    @Test
    fun `searchViewModel public and internal member count never increases`() {
        val file = viewModelSource()
        assertTrue(file.isFile, "SearchViewModel.kt not found at ${file.path}")
        ...
        assertTrue(
            ...
            "SearchViewModel grew to ${members.size} public+internal members (ceiling " +
                "$maxPublicInternalMembers):\n${members.joinToString("\n") { it.trim() }}\n" +
                "New behaviour belongs in an extracted module (the SeerrRequestStateHolder / QuickDownloadActions shape) wired through " +
