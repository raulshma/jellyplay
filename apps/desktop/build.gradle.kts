...
// Release channel for the desktop auto-update decision
// (docs/adr/desktop-auto-update.md): "release" only when -PjellyplayVersion
// was EXPLICITLY passed — i.e. a CI release lane produced this build. The
// "0.1.0" dev-machine fallback (and any bare `gradlew run`/IDE build) is
// channel=dev, and the update check then reports "up to date" BY
// CONSTRUCTION (no feed fetch can offer an update to a build that was never
// part of a release channel) — this replaces the old 999999.0.0 sentinel's
// job with an explicit, generated flag instead of a fake version number.
val jellyplayReleaseChannel =
    if (providers.gradleProperty("jellyplayVersion").isPresent) "release" else "dev"
...
val writeDesktopBuildInfo = tasks.register<WriteDesktopBuildInfoTask>("writeDesktopBuildInfo") {
    ...
    channel.set(jellyplayReleaseChannel)
