 ...
 * object is Koin-agnostic. The step ledger / poller / auto-exit / report
 * chassis is the shared [HarnessRunner]; the Robot evidence helpers (incl.
 * the focus diagnostics and the noWindowToFront knob) are the shared
 * [HarnessRobot].
 *
 * Properties:
...
object DesktopSessionHarness {
    ...
    /**
     * One session's flow steps + harness-specific config. The step ledger,
     * awaitUntil poller, auto-exit timer and report writer are the shared
     * [HarnessRunner]; the Robot evidence helpers are the shared
     * [HarnessRobot]. The process always exits 0 (the report's overallPass
     * is the verdict — the runner script greps it).
     */
    private class SessionRunner(private val deps: SessionHarnessDeps) {
        ...
        suspend fun run() {
            ...
            val navOk = !fatalStop && runner.step("NAV_READY") {
                runner.awaitUntil(30_000) { backStackProvider != null } ||
            ...
            if (!fatalStop) runner.step("SCREENSHOT_HOME") { robot.screenshot("signed-in-home") }

            val pushOk = !fatalStop && runner.step("PUSH_PLAYER") {
                val stack = currentBackStack() ?: fail("no back stack")
                stack.add(Route.VideoPlayer(itemId = itemId))
                runner.awaitUntil(5_000) { stack.lastOrNull() is Route.VideoPlayer } ||
            ...
            val engineOk = !fatalStop && runner.step("ENGINE_CREATED") {
                runner.awaitUntil(45_000) {
            ...
            if (!fatalStop) runner.step("SCREENSHOT_PLAYER_OPEN") { robot.screenshot("player-open") }

            val playbackOk = !fatalStop && runner.step("PLAYBACK") {
                runner.awaitUntil((90_000 - elapsedSince(pushAtMs)).coerceAtLeast(1_000)) {
                    engine = deps.engineRecorder.latestVideoEngine()
            ...
            if (!fatalStop) runner.step("SCREENSHOT_MID_PLAY") { robot.screenshot("mid-play") }
            ...
            runner.step("SHEET_TRIGGER_SCAN") {
                mapOf(
            ...
            val overlayOk = !fatalStop && runner.step("OVERLAY_SPACE") {
                ...
                while (true) {
                    ...
                    robot.injectKey(KeyEvent.VK_SPACE, "OVERLAY_SPACE attempt $attempts") ||
                        fail("SPACE injection failed (Robot)")
                    toggled = runner.awaitUntil(SPACE_TOGGLE_PROBE_MS) {
                ...
                robot.screenshot("player-controls-overlay", "overlay-space")
            ...
            runner.step("ESC_SEQUENCE") {
                ...
                robot.injectKey(KeyEvent.VK_ESCAPE, "ESC_SEQUENCE") || fail("ESC injection failed (Robot)")
                val popped = runner.awaitUntil(10_000) { stack.size < before }
        ...
        private suspend fun ensurePlayerRouteTop(): Map<String, String> {
            ...
            stack.add(Route.VideoPlayer(itemId = itemId))
            runner.awaitUntil(5_000) { currentBackStack()?.lastOrNull() is Route.VideoPlayer } ||
